...
1. 母串Ｓ的第一个字符和模式串Ｔ的第一个字符进行比较
2. 如果相等，则继续比较下一个字符是否相等
3. 如果不相等，则选择母串Ｓ的第二个字符和模式串Ｔ的第一个字符进行比较
4. 重复步骤２和３，直至在母串Ｓ中匹配到模式串Ｔ或者枚举到母串末尾
...
next数组：如果子串有相同的前缀和后缀，则next[i]记录最长前缀里最后一个字符的下标位置（前缀和后缀不包括子串本身）；如果不存在相同的前缀和后缀，则next[i] = -1。
...
void get_next(char *pattern, int *next) {
    ...
    for (int i = 1, match = -1; pattern[i]; i++) {
        //循环找到一个match,使得match后的一位字符和当前的字符是一样的
        //直到match为-1为止
...
int find(char *buffer, char *pattern, int *next) {
    for (int i = 0, match = -1; i < strlen(buffer); i++) {
        //当match的后一位字符和母串的当前字符不相等时，则更新match为next数组存的值
        //直到match小于0为止
...
常用于大量字符串的检索、去重、排序等，时间复杂度O(n)，n为查找字符串的长度

特点：

1. Trie树的根结点上不存储字符，其余结点上存且只存一个字符
2. 从根结点出发，到某一结点经过的字符，即是该节点对应的前缀
3. 每个结点的孩子结点存储的字符各不相同
4. Trie树牺牲空间来换取时间，当数据量很大时，会占用很大空间。如果字符串均由小写字母组成，则每个结点最多会有２６个孩子结点，则最多会有26^n个用于存储的结点，n为字符串的最大长度

Trie树的根结点不存储字符

在Trie树中，不会存在某个节点存储着两个相同字符的孩子结点
...
三个过程：建立Trie树、建立失败指针、字符串匹配

广度优先搜索构造失败指针：每个节点都有一个失败指针，首先将根结点的失败指针指向空，根结点的直接子节点的失败指针指向根结点。每个节点的失败指针都是由它父节点的失败指针决定的。
...
森林没有中序遍历，原因：中序遍历只存在于二叉树中，森林中的树都是一般树，无法区分左右孩子

#### 并查集（不相交集合）

最坏时间复杂度O(N)：每次合并对应到森林上都是一个点连到一条链的一端，此时森林相当于一个链表

通常用森林来表示并查集，一棵有根树表示一个集合

查询：

1. 寻找当前节点的父节点
2. 如果父节点是它本身，则该父节点为树的根结点，直接返回，否则返回步骤１

合并：

1. 分别获得传入的两个节点所在的树的根结点
2. 如果两个根结点相同说明它们在一棵树上，返回false，结束合并操作
3. 如果两个根结点不同，则将其中一个根节点的父指针指向另一个根结点，合并操作完成，返回true

按秩合并：

1. 利用一个数组保存每个结点的所在树的节点总数，即保存每个结点的秩
2. 分别获得传入的两个结点所在的树的根节点
3. 比较两个根结点是否相同，相同则返回false，结束合并操作
4. 若两个根节点的秩不相同，比较它们秩的大小
5. 将秩较小的根结点的父指针指向秩较大的根结点
6. 更新合并后的根节点的秩，返回true，结束合并操作

路径压缩优化：在查找根结点时，将待查找的结点的父指针指向它所在的树的根结点
...
void build_automaton(Trie root) {
    root->fail = root;   //根结点的失败指针指向自身
    TrieNode *q[MAX_SIZE];   //代表数列的数组
    int l = 0, r = 0;   //队首、队尾
    q[r++] = root;   //将根结点加入队列，更新r
    while (l < r) {
        TrieNode *now = q[l++];
        for (int i = 0; i < SIZE; i++) {
...
int find_set(DisjointSet *s, int node) {
    //如果node是所在集合的代表元素，也就是对应有根树的根的话，就直接返回node的值；
    //但如果node不是所在集合的代表元素，就需要不断递归node的父节点，直到找到集合的代表元素为止
...
二叉查找树：要么是空树，要么对任意结点，如果左子树不为空，则左子树上所有结点的权值都小于该节点的权值；如果右子树不为空，则右子树上所有结点的权值都大于该节点的权值

二叉查找树插入查找的效率都是O(logn)，最坏情况是O(n)

插入操作：

1. 根结点为空则新元素直接作为根节点，否则传入的参数value与根节点进行比较
2. value等于当前结点则直接返回，小于则跳转到步骤3，而如果value大于当前节点时，跳转到步骤4
3. 判断当前结点是否存在左孩子，如果存在则让其左孩子继续调用插入方法，回到步骤2；如果不存在则将新元素插入到当前节点的左孩子位置上
4. 判断当前结点是否存在右孩子，如果存在则让其右孩子继续调用插入方法，回到步骤2；如果不存在则将新元素插入到当前节点的右孩子位置上
...
1. 找到当前结点的左孩子，如果当前结点没有左孩子则不存在前驱，若存在，则找到其左孩子的右孩子
2. 若当前节点有右孩子则继续寻找其右孩子，重复步骤二，直到找到一个节点不存在右孩子时，那么它就是我们要找的前驱

删除操作：

1. 如果当前节点仅存在左孩子，则更新左孩子的父节点为被删除节点的父节点
2. 如果当前节点仅存在右孩子，则更新右孩子的父节点为被删除节点的父节点
3. 更新被删除结点的父节点的孩子指针，使其原本指向被删除结点的指针指向被删除结点的孩子节点

完整的删除操作：（维护二叉查找树的特性）

1. 找到被删除的节点
2. 若它存在左孩子，则找到他的前驱，用前驱替换被删除节点的值，在调用删除节点的方法删除前驱
3. 若被删除节点不存在左孩子，则找到他的后继，用后继替换被删除节点的值，再调用删除节点的方法删除后继
4. 若被删除的节点不存在孩子结点，直接调用删除结点的方法删除它
>>>>>>> 15705cb11374a64e6630a4e35c437c30fab24eb0
